...
if [ -n "$show" ]; then
	volume_amount="$(wpctl get-volume @DEFAULT_SINK@ | head -n 1 | awk '{print substr($5, 1, length($5)-1)}')"
	background_color='#000000FF'
	border_color='#FFFFFFFF'
	bar_color='#FFFFFFFF'
	# if muted, change the background color to red
	[ "$(wpctl get-mute @DEFAULT_SINK@ | awk '{print $2}')" == 'yes' ] && background_color='#FF000088'
